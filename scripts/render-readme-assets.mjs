...
function activeProjects() {
  const card = (x, status, dot, title, summary, lines) => `<g transform="translate(${x} 96)"><rect width="350" height="285" rx="18" class="p"/><circle cx="28" cy="30" r="5" fill="${dot}"/><text x="46" y="35" class="m">${status}</text><text x="24" y="74" class="c">${title}</text><text x="24" y="104" class="sum">${summary}</text>${lines.map((line, i) => `<text x="24" y="${146 + i * 30}" class="s">${line}</text>`).join('')}</g>`;
  return shell(1200, 430, `<g class="ui"><rect x=".5" y=".5" width="1199" height="429" rx="22" fill="none" stroke="rgba(255,255,255,.10)"/><g transform="translate(36 30)"><path class="o" d="M0 18h8l4-14 6 28 5-18h8"/><path class="o" d="M0 38h34"/><text x="52" y="27" class="h">Active Projects</text><text x="52" y="49" class="m">technical build focus · hardware · infra · web systems</text></g>${card(36, 'ACTIVE', C.green, '60 GHz mmWave Radar', 'Contactless physiology for sleep/seated health monitoring.', ['• UART radar frame parser · checksum handling', '• HR/BR/presence/range telemetry extraction', '• VisLog API · live browser dashboard stream', '• ESP32-C6 WiFi provisioning · captive portal', '• nRF52840 BLE bridge + ESP32 bring-up'])}${card(425, 'FINALIZED', C.blue, 'Home Lab Server', 'Local-first storage, backups, services, and private AI.', ['• Proxmox VE virtualization · VMs/LXC layout', '• TrueNAS datasets · snapshots · backup policy', '• Nextcloud · Immich · local data organization', '• Docker Compose services · ClawCode tooling', '• Raspberry Pi edge services + monitoring'])}${card(814, 'ACTIVE', C.green, 'Professional Profile', 'Recruiter-facing documentation and portfolio automation.', ['• Astro · HTML/CSS · GitHub Pages deployment', '• GitHub Actions CI/CD · release assets', '• README SVG generation · cache busting', '• Plausible/referrer analytics pipeline', '• Portfolio tracking for recruiter traffic'])}</g>`);
}
