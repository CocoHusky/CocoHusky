...
function activeProjects() {
  const card = (x, title, sections, state = 'ACTIVE') => `<g transform="translate(${x} 96)"><rect width="350" height="285" rx="18" class="p"/><circle cx="28" cy="30" r="5" fill="${state === 'ACTIVE' ? C.green : C.orange}"/><text x="46" y="35" class="m">${state}</text><text x="24" y="74" class="c">${title}</text>${sections.map(([label, lines], sectionIndex) => { const y = 108 + sectionIndex * 74; return `<text x="24" y="${y}" class="k">${label}</text>${lines.map((line, i) => `<text x="24" y="${y + 22 + i * 24}" class="s">${line}</text>`).join('')}`; }).join('')}</g>`;
  return shell(1200, 430, `<g class="ui"><rect x=".5" y=".5" width="1199" height="429" rx="22" fill="none" stroke="rgba(255,255,255,.10)"/><g transform="translate(36 30)"><path class="o" d="M0 18h8l4-14 6 28 5-18h8"/><path class="o" d="M0 38h34"/><text x="52" y="27" class="h">Active Projects</text><text x="52" y="49" class="m">technical build focus · hardware · infra · web systems</text></g>${card(36, '60 GHz mmWave Radar', [['USE CASE', ['Contactless physiology during sleep/sitting']], ['SYSTEM', ['UART frame decode · VisLog API layer', 'HR/BR/presence/range telemetry stream']], ['EMBEDDED', ['ESP32-C6 WiFi portal + live web UI', 'nRF52840 BLE + ESP32 bridge bring-up']]])}${card(425, 'Home Lab Server', [['USE CASE', ['Local-first data storage + private AI tools']], ['INFRASTRUCTURE', ['Proxmox VE VMs/LXC · Docker services', 'TrueNAS datasets · Nextcloud · Immich']], ['OPERATIONS', ['ClawCode · backups · maintenance plans', 'Raspberry Pi edge services + monitoring']]])}${card(814, 'Professional Profile', [['USE CASE', ['Document achievements for recruiters/self']], ['WEB STACK', ['Astro · HTML/CSS · GitHub Pages', 'GitHub Actions CI/CD + release assets']], ['ANALYTICS', ['Plausible-style referrer tracking', 'README SVG generation + cache busting']]], 'IN PROGRESS')}</g>`);
}
