...
const C = {
  bg: '#010409', bg2: '#06101c', bg3: '#0d1117', text: '#f5f7fb', text2: '#b6c2d4',
  text3: '#7e8a9b', orange: '#e8834a', orange2: '#ffb26d', blue: '#4c8dff', cyan: '#6fd3ff', green: '#22c55e'
};
...
async function main() {
  ...
  const rendered = { 'hero-banner.svg': await hero(), 'section-divider.svg': await divider(), 'active-projects-card.svg': await activeProjects() };
  await Promise.all(Object.entries(rendered).map(([file, svg]) => writeFile(path.join(out, file), `${svg}\n`, 'utf8')));
}
