...
function activeProjects() {
  const card = (x, title, lines, state = 'ACTIVE') => `<g transform="translate(${x} 96)"><rect width="350" height="180" rx="18" class="p"/><circle cx="28" cy="30" r="5" fill="${state === 'ACTIVE' ? C.green : C.orange}"/><text x="46" y="35" class="m">${state}</text><text x="24" y="74" class="c">${title}</text>${lines.map((line, i) => `<text x="24" y="${108 + i * 24}" class="s">${line}</text>`).join('')}</g>`;
  return shell(1200, 320, `<g class="ui"><rect x=".5" y=".5" width="1199" height="319" rx="22" fill="none" stroke="rgba(255,255,255,.10)"/><g transform="translate(36 30)"><path class="o" d="M0 18h8l4-14 6 28 5-18h8"/><path class="o" d="M0 38h34"/><text x="52" y="27" class="h">Active Projects</text><text x="52" y="49" class="m">current technical build focus</text></g>${card(36, '60 GHz mmWave Radar', ['API layer · VisLog telemetry', 'WiFi provisioning · embedded hardware', 'nRF52840 + ESP32 bring-up'])}${card(425, 'Home Lab Server', ['TrueNAS · Nextcloud · Immich', 'ClawCode · backup maintenance', 'Raspberry Pi services + monitoring'])}${card(814, 'Professional Profile', ['Astro · HTML/CSS · GitHub workflows', 'portfolio deployment automation', 'visitor/referrer tracking pipeline'], 'IN PROGRESS')}</g>`);
}
